...
// 			img:  Pointer to array representing image to be transformed of dimensions Nx times Ny times Nz (img_shape=Nx times Ny)
//			ofs:  Buffer containing geometric informations concerning the projection-directions (angular information)
//				  The first two entries are xd,yd in direction along the detector-line with length delta_xi
//				  Third and fourth (qx,qy) from origin to source with length RE
//				  Fifth and sixth (dx0,dy0) from origin to center of detector-line (orthogonal projection of origin onto detectorline)
//			sdpd: Buffer containing the values associated to sqrt(xi^2+R^2) as weighting
//			Geometry_information:  Contains various geometric quantities relevant for the computation, more precisely
//					0. R/delta_x, 1. RE/delta_x, 2. delta_xi/delta_x, 3.x_mid (in image_pixels),
// 					4. y_mid (in image_pixels), 5 xi_midpoint (in detector_pixels),
// 				  6. Nx, 7.Ny, 8.Nxi, 9.Nphi, 10.delta_x
// Output:
						       														 				 ...
						       														 				 __constant real* Geometryinformation)
{
  ...
  real2 dm = d0 + d*(-midpoint_det+s-1) - q;
    //dm = (rea	l2)(d0.x+d.x*(-midpoint_det+s-1)-q.x, d0.y+d.y*(-midpoint_det+s-1)-q.y);
  ...
  for (int y=0;y<Nyy;y++)
    {
      ...
      // for (x,ylowint) compute t and ss orthogonal distances from source (t values in (0,1))
      // and projected detectorposition (divided by delta_xi )
...
//				  Third and fourth (qx,qy) from origin to source with length RE
//				  Fifth and sixth (dx0,dy0) from origin to center of detector-line
//			sdpd: Buffer containing the values associated to sqrt(xi^2+R^2) as weighting
//			Geometry_information:  Contains various geometric quantities relevant for the computation, more precisely
//					0. R/delta_x, 1. RE/delta_x, 2. delta_xi/delta_x, 3.x_mid (in image_pixels),
// 					4. y_mid (in image_pixels), 5 xi_midpoint (in detector_pixels),
