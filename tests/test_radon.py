...
def test_projection():
    ...
    print("Average time required for backprojection", (time.perf_counter() - a) / M)
    ...
    if PLOT:
        ...
        PS.show_geometry(np.pi * 3 / 8.0, axes=plt.subplot(2, 2, 4))
...
def test_weighting():
    ...
    img = np.zeros([N, N])
    img[int(N / 3.0) : int(2 * N / 3.0)][:, int(N / 3.0) : int(2 * N / 3.0)] = 1
    ...
    mass_sino_rdm = np.sum(sino_gpu.get()[:, np.random.randint(0, angles)]) * PS.delta_s

    # Announce how many errors occurred
    print(
        "The mass inside the image is "
        + str(mass_image)
...
def test_adjointness():
    ...
    sino2_gpu = cl.array.zeros(queue, PS.sinogram_shape, dtype=dtype, order=order)
    ...
    for i in range(100):
        ...
        pairing_sino = (
            cl.array.vdot(gratopy.weight_sinogram(sino1_gpu, PS), sino2_gpu).get()
            * PS.delta_s
        )
    ...
    print(
        ...
        + str(count)
        + " out of"
        + " 100 tests adjointness-errors were bigger than"
        + str(eps)
    )
    assert len(Error) < 10, (
        "A large number of experiments for adjointness\
        turned out negative, number of errors: "
        + str(count)
        + " out of 100\
        tests adjointness-errors were bigger than "
        + str(eps)
    )
...
def test_limited_angles():
    ...
    evaluate_control_numbers(
        ...
        expected_result=340.93,
        classified="sino",
        name="second sinogram with correct " + "fullangle setting",
    )

    evaluate_control_numbers(
        backprojected_correct,
        (N, N, Ns, len(angles_correct), 2),
        expected_result=3385.137,
        classified="img",
    ...
    evaluate_control_numbers(
        ...
        expected_result=3385.137,
        classified="img",
        name="second backprojected image" + "with correct fullangle setting",
    )
...
def test_nonquadratic():
    ...
    PS = gratopy.ProjectionSettings(queue, gratopy.PARALLEL, img_gpu.shape, angles, Ns)
...
def test_create_sparse_matrix():
    ...
    # define projectionsetting
    PS = gratopy.ProjectionSettings(
        queue, gratopy.PARALLEL, img.shape, angles, n_detectors=number_detectors
    )
...
def test_angle_input_variants():
    ...
    Angles.append(
        list(np.linspace(0, np.pi / 3, 101)[:-1])
        + list(np.linspace(np.pi / 3, np.pi * 2.0 / 3, 51)[:-1])
        + list(np.linspace(np.pi * 2 / 3, np.pi, 101)[:-1])
    )
    Angles_expected.append(
        list(np.arange(0, np.pi / 3 - 0.00001, np.pi / 300))
        + list(np.arange(np.pi / 3, np.pi * 2 / 3 - 0.00001, np.pi / 150))
        + list(np.arange(np.pi * 2 / 3, np.pi - 0.00001, np.pi / 300))
    )
    Angle_weights_expected.append(
    ...
    Angles.append(
        [
            (list(np.linspace(0, np.pi / 2, 101)[:-1] + np.pi / 4 / 100), 0, np.pi / 2),
    ...
    Angles_expected.append(list(np.arange(np.pi * 1 / 400, np.pi - 0.00001, np.pi / 200)))
    ...
    Angles_expected.append(
        list(np.arange(np.pi / 300, np.pi / 3 - 0.00001, np.pi / 150))
        + list(np.arange(np.pi * 201 / 300, np.pi - 0.00001, np.pi / 150))
    )
    ...
    Angles_expected.append(np.flip(np.arange(np.pi / 200, np.pi - 0.00001, np.pi / 100)))
    Angle_weights_expected.append(list(np.ones(100) * np.pi / 100))
    ...
    for j in range(len(Angles)):
        ...
        assert np.linalg.norm(PS.angles - np.array(Angles_expected[j])) < 0.01, (
            " The angles in " + str(j) + ".th " + "case-studie were not as expected"
        )
        assert (
            np.linalg.norm(PS.angle_weights - np.array(Angle_weights_expected[j])) < 0.001
        ), (
            " The angles_weights in "
            + str(j)
            + ".th "
    ...
    for j in range(len(Angles)):
        ...
        for angle_weights in [1, 1.0, np.ones(na), list(np.ones(na)), None]:
